<template>
    <div class="phone-form">
      ...
      <form @submit.prevent="handleSubmit" novalidate class="!mt-24">
        <div class="flex flex-col pb-10">
          ...
          <Password v-model="form.password" toggleMask @input="validatePassword" @blur="validatePassword">
            <!-- Password input component -->
          </Password>
          <div class="mt-4">
            <p :class="minLengthClass" class="text-sm">Minimum 8 digits</p>
            <p :class="letterNumberClass" class="text-sm">Minimum 1 letter and 1 number</p>
          </div>
        </div>
  ...
  const validatePassword = () => {
    ...
    const letterNumberValid = /[a-zA-Z]/.test(form.value.password) && /[0-9]/.test(form.value.password);
