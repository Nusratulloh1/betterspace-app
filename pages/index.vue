<template>
  <div class="main">
    ...
    <InputGroup class="mt-6 !rounded-2xl">
      <InputText size="large" class="!bg-[#FFFFFF] !border-none"
        placeholder="Search for nutrition, gym, gift and etc..." v-model="value1" @keyup.enter="submitSearch" />
      <Button @click="submitSearch" class="!bg-[#FFFFFF] text-[#B2B5B8] !border-none" icon="pi pi-search" />
    </InputGroup>
    <div class="flex">
      <Button @click="feedbackModal = true"
        class="!bg-[#FFFFFF] !shadow-sm !mx-auto mt-6 !text-[#6D6D6D] !rounded-[12px]" text
        ...
        <template #icon>
          <svg width="20" height="20" viewBox="0 0 26 25" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
              ...
              fill="#6D6D6D" stroke="#6D6D6D" stroke-width="0.390077" />
          </svg>
        </template>
      </Button>
    </div>
    ...
    <Dialog v-model:visible="feedbackModal" header=" " modal :style="{ width: '25rem' }">
      <feedbackForm />
    </Dialog>
  </div>
</template>
